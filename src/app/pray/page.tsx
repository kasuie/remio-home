 ...
 * @LastEditors: kasuie
 * @LastEditTime: 2024-06-17 11:55:30
 * @Description:
 */
"use client";
...
export default function Pray() {
  ...
  let countFour = 0, allCountFour = 0
    ...
    while (count > 0) {
      ...
      if (count == 1) {
        ...
        if (_nofour >= 9) {
          ...
          if (random < 0.15 && allCountFour < fours[nameMaps[type]]) {
            ++_nofour;
            ++allCountFour;
            if (type == "c") {
              const index = Math.floor(Math.random() * fourSatrs.length);
  ...
  const reduceData = (versionss: any, arr: any, type: string) => {
    console.log(allCountFour, countFour);
  ...
  const onClick = () => {
    ...
    fetch(`/apis/ys/getWishByVs?${queryParams}`, {
      ...
      .then((res: any) => {
        ...
        console.log(roleJson.data.filter((v: any) => v.rank_type == "4").length);
        allCountFour = 0;
        countFour = 0;
        // const roleJson = { data: [] };
        const weaponJson = reduceData(versionss, weapon, "w");
        console.log(weaponJson.data.filter((v: any) => v.rank_type == "4").length);
        allCountFour = 0;
        countFour = 0;
        const commonJson = reduceData(versionss, common, "c");
        console.log(commonJson.data.filter((v: any) => v.rank_type == "4").length);
